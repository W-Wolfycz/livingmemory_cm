...
LivingMemoryCM 是基于上游 LivingMemory 2.5.7 的 CM-only 修改版。本条汇总本 fork
相对上游的公开差异；其后的版本记录保留自上游原始 CHANGELOG。

### 修改

- 要求 `chat_memory >= 1.1.1` 开启 `context_takeover`，由 ChatMemory 管理短期上下文和
  原始消息，本插件只维护长期记忆；移除上游自管 conversation history 与 BM25 文档路。
- 反思链路改用 ChatMemory 会话查询、严格 `(created_at, id)` 复合游标、稳定批次 ID、
  幂等写入和 session/persona 隔离，并对群聊身份和 AstrBot 人格解析做一致性适配。
- 采用中性摘要、关键事实、紧凑注入和记忆原子；增加召回阈值、近期记忆槽位、重要记忆
  衰减保护和可选 Agent 主动检索/写入工具。
- 为 SQLite/FAISS 增加 Embedding 指纹、索引代际、一致性检查和影子重建；图向量压缩为
  每条来源记忆一个聚合向量，升级或重建失败时保留旧索引和正式数据。
- Dashboard 追踪上游 2.5.7 的可移植视觉和稳定性改进，保留 CM persona 透传、批量编辑/
  删除与大图渲染优化，并改为中文单语言；修复全屏图谱工具栏覆盖和提示文字裁切。
- 不移植与 CM 后端不匹配的 PromptManager、JSON/CSV 导入导出、原始来源编辑、重新总结
  和结构化主题/关键事实编辑入口。
- 统一公开版本为 `2.5.7-cm`，补齐 AGPL-3.0 修改声明、当前源码入口和 Lucide 第三方许可；
  本地代码级回归为 `435 passed`，AstrBot 与真实 Provider 仍由部署端验收。
- 公开文档收敛为当前架构、CM 集成契约、配置和测试四份主文档；Release 随附覆盖本地
  数据、虚拟环境、缓存和旧压缩包的 `.gitignore`，避免提交运行数据或开发残留。

### 升级说明

- Release 包不包含 `data/`、数据库或索引；升级时保留部署端现有数据，并建议先做离线备份。
- 插件会在检测到版本变化时创建版本备份；旧数据库和旧图向量不会在导入阶段被静默删除。
